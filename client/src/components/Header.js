import { createStyles, Box, Button,  Group,  Header,  Image,
         Burger, ActionIcon, Avatar, Text, Menu,} from "@mantine/core";
...
//import imagen from "./assets/pizzabig01.jpg";

// config de los estilos de mantine
const useStyles = createStyles((theme) => ({
  ...
  hiddenDesktop: {
   ...
   useMemo(() => {
    ...
    setInterval(() => {dispatch(refreshToken());}, 5 * 60 * 1000);}, [dispatch]);
...
/***use-hotkeys accepts as its first argument an array of hotkeys and handler tuples:
   ...
   return (
    <Box sx={{ position: "relative", zIndex: 999 }}>
      <Header sx={(theme) => ({backgroundColor: theme.colorScheme === "dark" ? "#292c2f" : "#fff",})}
        height={60} px="md" >
        <Group position="apart" sx={{ height: "100%" }}>

          <Link to="/">
            <Image width={60} height={55}src={require(`../assets/pizzabig02.jpg`)}/>
          </Link>
          ...
          <Group>
            ...
            <Group>
              ...
              <ActionIcon sx={(theme) => ({ color: theme.colorScheme === "dark" ? "yellow" : "#3b5bdb",
                })} onClick={() => onToggleColorScheme()} size="lg">
            ...
            <Menu opened={opened} transition="pop-top-right" position="bottom-start" offset={10}
              transitionDuration={200}>

              {isRefreshingToken && loggedUser &&
              ...
              {!isRefreshingToken && Object?.entries(user)?.length > 0 && (
                <Menu.Target>
                  <Group sx={{cursor: "pointer", }} spacing={7}
                    onClick={toggle}>
              ...
              {!loggedUser && Object?.entries(user)?.length === 0 && (
                <Menu.Dropdown py="md">
                  ...
                  <Link style={{ textDecoration: "none",}}
                    to="/auth/register" onClick={() => close()}>
                    <Menu.Item>
                      <Button>Register</Button>
                    </Menu.Item>
                  </Link>
              ...
              {!isRefreshingToken && Object?.entries(user)?.length > 0 && (
                <Menu.Dropdown py="md">
                  ...
                  <Menu.Item onClick={() => {close(); dispatch(logout());}}
                    icon={<IoIosLogOut />} color="red">
                    Log Out
                  </Menu.Item>
                </Menu.Dropdown>)}
