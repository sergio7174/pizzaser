...
const UserInfo = () => {
  ...
  return (
    <form onSubmit={form.onSubmit(handleSubmit)}>
      ...
      <Paper sx={(theme) => ({ [theme.fn.smallerThan("md")]: {
            marginTop: 60,}, backgroundColor: theme.colorScheme === "dark" ? theme.colors.dark[6] : "#fff", })} my="md" shadow="xs" radius="md" p="lg" withBorder >
        {user?.loginType === "local" && (
          ...
          <Group p="lg">
            ...
            <FileInput ref={fileInputRef} placeholder="Pick picture" label="Your avatar"
              withAsterisk sx={{ display: "none", }}
        ...
        <Group mb="lg" position="center" grow>
          ...
          <TextInput placeholder="Your first name" label="First Name"
            {...form.getInputProps("firstName")}
            sx={(theme) => ({ input: {background:
                  theme.colorScheme === "dark" ? "#1f2124" : "#e9e9ec",},})}/>
          
          <TextInput placeholder="Your last name" label="Last Name" sx={(theme) => ({
              input: { background: theme.colorScheme === "dark" ? "#1f2124" : "#e9e9ec",},
            })}
        ...
        <Group mb="lg" position="center" grow>
          ...
          <TextInput placeholder="Your email address" label="Email Address"
            sx={(theme) => ({ input: { background: theme.colorScheme === "dark" ? "#1f2124" : "#e9e9ec", }, })} {...form.getInputProps("email")}/>
          
          <TextInput placeholder="Your username" label="Username" sx={(theme) => ({
              input: { background: theme.colorScheme === "dark" ? "#1f2124" : "#e9e9ec",
              },  })}
        ...
        {user?.providers?.includes("local") && (
          ...
          <PasswordInput mb="md" placeholder="Current password" label="Current Password"
            styles={(theme) => ({ background: theme.colorScheme === "dark" ? "#1f2124" : "#e9e9ec",
            })}
            {...form.getInputProps("currentPassword")}/>)}

        <PasswordInput mb="md" placeholder="New password" label="New Password"
          {...form.getInputProps("newPassword")}/>

        <PasswordInput mb="md" placeholder="Confirm new password"
          label="Confirm New Password"
          {...form.getInputProps("confirmPassword")}/>
        
        <Button loading={updateUserState.loading}
          type="submit" mt="md"  radius="md" shadow="sm" >
          Save Changes
        </Button>
      </Paper>
    </form>
