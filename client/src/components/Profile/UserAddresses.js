            ...
            {userAddresses.map((address) => {
              return (
                <Grid.Col key={address?._id} xs={4}>
                  ...
                  <Card p="lg" shadow="lg" radius="md" sx={(theme) => ({
                      backgroundColor: theme.colorScheme === "dark" ? theme.colors.dark[7]
                          : "#f8f9fa", borderWidth: 2, borderStyle: "solid", borderColor:
                        theme.colorScheme === "dark"
                          ? "transparent"
                          : "#f1f5f8", })} >
                    <Group mb="md" position="apart">
                      
                      <Title order={4} sx={(theme) => ({ fontFamily: theme.fontFamily,})}>
                        {address?.firstName} {address?.lastName}
                      </Title>
                      
                      {address?.isPrimary && (

                    <Badge sx={(theme) => ({ color: theme.colorScheme === "dark" ? "#fff" : "#000",})}>
                          Default
                    </Badge>)}
