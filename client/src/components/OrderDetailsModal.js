...
const OrderDetailsModal = ({ order, subTotal }) => {
  return (
    ...
    <Paper sx={(theme) => ({ [theme.fn.smallerThan("md")]: { marginBottom: 120,},
        backgroundColor:
          theme.colorScheme === "dark" ? theme.colors.dark[6] : "#fff",
      })} shadow="xs" radius="md" p="md" >
      
      <Title sx={(theme) => ({ fontFamily: theme.fontFamily, paddingBottom: 16,
          marginBottom: 16, borderBottom: `1px solid ${theme.colorScheme === "dark"
              ? theme.colors.gray[3]
              : theme.colors.dark[7]
          }`,
        })}
      ...
      {order?.cart?.cartItems?.map((item) => (
        <Group key={item._id} mb="md" mt="md">
          ...
          <Image radius="md" src={item.pizza.image} width={100} height={100} withPlaceholder/>
