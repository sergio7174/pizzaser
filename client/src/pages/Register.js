...
const useStyles = createStyles((theme) => ({
  ...
  form: {borderRight: `1px solid ${theme.colorScheme === "dark" ? theme.colors.dark[7] : theme.colors. gray[3]
    }`,
    minHeight: `100vh`,
    maxWidth: `50%`,
    paddingTop: 80,
    ...
    <div className={classes.wrapper}>
      <Paper sx={(theme) => ({backgroundColor: theme.colorScheme === "dark" ? "#0e1012" : "#e9e9ec",})}
        ...
        <form onSubmit={form.onSubmit(handleSubmit)} id="register-form">
          ...
          <TextInput withAsterisk label="First Name" name="firstName" placeholder="John"
            disabled={loading} size="md" {...form.getInputProps("firstName")}/>

          <TextInput withAsterisk label="Last Name" name="lastName" placeholder="Doe"
            disabled={loading} size="md" mt="md"
            {...form.getInputProps("lastName")}/>
          
          <TextInput withAsterisk label="Username" name="username" placeholder="doe28"
            disabled={loading} mt="md" size="md"
            {...form.getInputProps("username")} />

          <TextInput withAsterisk label="Email address" name="email" placeholder="someone@example.com"
            disabled={loading} mt="md" size="md"
            {...form.getInputProps("email")}/>
          
          <PasswordInput withAsterisk label="Password" name="password" placeholder="**********"
            disabled={loading} mt="md" size="md"
            {...form.getInputProps("password")}/>
          
          <Checkbox label="Keep me logged in" disabled={loading} name="rememberMe"
            mt="xl" size="md" checked={form.values.rememberMe}
            onChange={(event) =>
