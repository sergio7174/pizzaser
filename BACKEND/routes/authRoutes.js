...
router.get("/auth/google", checkAuth, (req, res) => {passport.authenticate("google", {
    
  session: false,scope: ["profile", "email"],})(req, res);});


router.get("/auth/google/callback",(req, res, next) => {passport.authenticate(
      "google", {session: false,}, (err, user, info) => {
        if (err) {res.cookie("info", err.toString(), {maxAge: 1000 * 5,});
  ...
  (req, res) => {const refreshToken = generateRefreshToken(req.user, "1d", "google");
